...
# __call__ must be a real ASYNC method: ASGI invokes app(scope, receive, send),
# special methods bypass instance __getattr__, and uvicorn's interface
# auto-detection picks ASGI3 only when __call__ is a coroutine function
# (uvicorn/config.py: iscoroutinefunction(app.__call__)).
class _AppProxy:  # pragma: no cover - deployment glue
    def __getattr__(self, name):
        return getattr(_build(), name)

    async def __call__(self, scope, receive, send):
        await _build()(scope, receive, send)
